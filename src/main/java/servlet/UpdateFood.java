...
public class UpdateFood extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			/*
			 * String sql = "update addFood set fname = '"+ f_name +"' fprice = '" + f_price
			 * +"' fcategory = '" +f_category +"' fdescription = '" + f_description +
			 * "'fimg ='"+fileName+"' where id='"+id+"'";
			 */
			String sql ="Update addFood set fname=?,fprice=?,fcategory=?,fdescription=?,fimg=? where id= "+ id;

			PreparedStatement ps = con.prepareStatement(sql);
			
			
			ps.setString(1, f_name);
			ps.setString(2, f_price);
			ps.setString(3, f_category);
			ps.setString(4, f_description);
			...
			if (i > 0) {
				...
				RequestDispatcher rd = request.getRequestDispatcher("Admin_ListOfFood.jsp");
				rd.forward(request, response);
			} else {
				session.setAttribute("Done", id);
				RequestDispatcher rd = request.getRequestDispatcher("Admin_ListOfFood.jsp");
				rd.forward(request, response);
			}
		} catch (Exception e) {
